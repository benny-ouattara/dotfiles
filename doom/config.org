#+TITLE: Config
#+PROPERTY: header-args :tangle ./config.el :results silent :lexical t
* Dependencies
...
#+end_src
* Setup
We start by setting up tangling on this config file into config.el
config.org will be changed and config.el will behave like a lock file, tracked
in version control but only a mirror of config.org
#+begin_src emacs-lisp
